...
export function AwsServicePanel({service}: AwsServicePanelProps) {
  return (
    <div data-slot="aws-service-panel" className="hidden lg:block">
      ...
      <h3 key={`${service.name}-title`} className="mt-6 animate-in text-xl font-medium text-foreground duration-500 fade-in">
        {service.name}
      </h3>
      <p className="mt-2 max-w-xl text-[0.95rem] leading-relaxed text-foreground/75">{service.description}</p>
    </div>
  );
}
